...
>
> v2의 `/sync-status`도 `TypeError`로 중단됩니다. 그리고 v2.x 기기 쪽에서는 아무것도 그것을 막지 못합니다 — v3.0.0이 기록하는 `min_reader_version` 표식은 그 값보다 **낮은** 버전의 읽는 쪽만 막고, **v2.x에는 그 표식을 읽는 코드가 없습니다.** 그 가드는 v3.0.0에서 처음 들어왔습니다. 배포 순서가 유일한 방어입니다.
...
그다음 아래 순서를 지킵니다. **모든 기기에서 `/sync-backup`을 실행하지 마세요.**

1. **한 기기에서만 백업합니다.** `/sync-backup` 한 번이 레포를 새 형식으로 옮깁니다.
2. **나머지 기기는 복원을 먼저 합니다.** 그 기기의 첫 `/sync-backup`보다 `/sync-restore`가 먼저입니다. 아직 공유 기준선이 없어 양쪽에 있는 항목이 모두 "양쪽이 모두 바뀌었습니다"로 나오고 항목마다 묻는데, 그것이 선택을 지키는 장치이지 결함이 아닙니다. 한 번 답하면 기준선이 기록되어 다음 실행부터는 조용해집니다.

기준선이 없는 기기에서 백업을 먼저 하면 **묻지 않습니다.** 병합이 합집합으로 물러나면서 **양쪽에 있는 항목은 로컬 값이 레포 값을 조용히 덮고, 충돌로 보고되지 않습니다.** 이 기기에 없던 항목은 그대로 보존되므로 삭제는 일어나지 않지만, 다른 곳에서 정해 둔 값이 아무 말 없이 바뀔 수 있습니다.
...
- **MCP 서버는 서버 이름 키 단위로 병합됩니다.** `mcp-servers.json`은 서버마다 따로 판정되므로, 한 기기의 백업이 다른 기기에만 있는 서버를 지우지 않습니다 — **모든 기기가 v3.0.0 이상일 때에 한합니다**(v2.x 백업은 그 서버를 지웁니다. 위의 *v3.0.0으로 올릴 때* 참조). 삭제는 전파되지만 로컬 제거는 `/sync-restore`가 서버마다 물어본 뒤에만 이루어집니다.
- **`plugins.json`은 키 단위로 병합됩니다.** 플러그인 항목·마켓플레이스·설정 키가 각각 판정되므로, 한 기기의 백업이 다른 기기에만 있는 항목을 지우지 않습니다 — **모든 기기가 v3.0.0 이상일 때에 한합니다**(v2.x 백업은 그 항목을 지웁니다. 위의 *v3.0.0으로 올릴 때* 참조). 삭제는 전파되지만 로컬 제거는 `/sync-restore`가 물어본 뒤에만 이루어집니다.
...
- **백업은 그 경로를 레포에서 지웁니다.** 건너뛰는 것이 아닙니다 — `/sync-backup`이 매치된 파일을 레포 작업 트리에서 지우고 그 삭제를 커밋합니다. **다른 기기가 같은 경로에 이미 올려 둔 사본도 함께 사라집니다.**
- **복원은 `.syncignore`를 무시합니다.** `/sync-restore`는 그 파일을 읽지 않으므로, 제외한 경로에 레포 파일이 있으면 다른 파일과 똑같이 복원 판정을 받습니다 — 새 파일이면 추가되고, 레포가 앞서면 덮어쓰고, 양쪽이 바뀌었으면 머지합니다. 의도한 동작입니다: 받는 방향에서도 존중하면 다른 기기가 그 경로에 올린 파일을 영영 받지 못합니다. **경로를 제외해도 로컬 파일이 덮어쓰이지 않게 보호되지는 않습니다** — 제외는 내 사본을 레포에 올리지 않을 뿐입니다.
- **`/sync-status`는 두 경우를 다르게 보고합니다.** 레포에 없는 제외 파일은 아예 나오지 않습니다. 레포에 남아 있는 제외 파일은 "`.syncignore` 제외인데 레포에 남아 있음 (backup 시 레포에서 삭제)"으로 따로 나옵니다 — push 예정이라 부르는 것도 거짓이고, 아무 말도 하지 않는 것도 거짓이기 때문입니다.
- **`plugins.json`·`mcp-servers.json`은 제외하지 못합니다.** 그 둘은 제외를 적용하는 단계보다 **뒤인** 단계가 다시 생성하므로 패턴을 적어도 효과가 없습니다. 그 둘의 민감한 값은 제외가 아니라 마스킹으로 가리며, 키 이름은 남습니다.
