...
# **셋 다 실측이다** — github은 `repo`(2차 스모크 9장), url·git은 **둘 다 `url`**이다
# (3차 스모크 13장. 로컬 http 서버를 세워 재야 나오는 갈래라 2차로는 만들 수 없었다).
# 문자열을 하나도 찾지 못하면 **인자를 만들 수 없음 = unrestorable**로 접는다 —
# 짐작해서 잘못된 인자를 넘기면 CLI가 모호한 문구로 실패해 사용자가 원인을 못 찾는다.
#
# git의 후보 둘 중 **첫째(`url`)가 실측이고 둘째(`repo`)는 방어로 남긴다.** 앞 판의 근거는
# *"어느 쪽이 옳은 필드인지 모르므로 둘 다 시도한다"* 였는데 그 근거는 3차 스모크로
# 사라졌다. **새 근거는 비대칭이다** — 지우면 `repo`만 가진 값(다른 CLI 판본이 썼거나
# 사용자가 손으로 적은 것)이 조용히 unrestorable이 되고, 남기는 비용은 0이다.
# 첫 후보가 실측으로 옳으므로 둘째가 실제로 뽑히는 일은 없다.
