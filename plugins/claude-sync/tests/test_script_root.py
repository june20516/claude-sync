...
**`sync-restore/SKILL.md` 5-1~5-4의 「버킷 → CLI 명령」 결속도 여기가 아니라
`test_restore_prose.py`에 있다.** 그것은 배선(무엇을 어떤 순서로 부르는가)이 아니라
**실행 산문의 의미**(어떤 버킷이 어떤 명령에 실리는가)라 셋째 관심사다. 아래
`SCRIPT_CONTRACT_PHRASES`는 계속 이 파일의 것이다 — 스킬이 스크립트의 어느 키를 읽는지를
거는 산문 앵커이고, 그쪽 파일은 그 키가 **어떤 행위에 묶이는지**를 건다.
"""
import json
import os
import re
