"""세 SKILL.md의 배선 계약 — SKILL.md를 **읽어서** 잰다.

거는 것: 호환성 검사의 위치와 그것이 앞서야 할 실행줄, 다운그레이드 탐지 순서와 그
결과를 렌더링하는 대화, 공유 스테이징을 비우는 횟수와 순서, base 게이트의 두 축,
restore 명령의 스코프 정책, 스킬이 스크립트의 어느 키를 읽는지를 적은 산문 앵커,
`## 동기화 대상` 절이 말하는 추출 필드, 선택 결과 JSON의 스키마, 그리고 그 표들이
스스로 줄어드는 것을 막는 완전성 메타가드.

그 계약을 재려면 프로덕션 어댑터의 상수가 필요해서 이 파일은 `plugin_config`·`mcp_config`를
import한다(선택 JSON의 섹션 이름, 보류 종류, 추출 필드 이름, 백업 문서의 relpath가 갖는
진실 원천). 4단계 bash와 같은 매칭 규칙을 대조하려고 `syncignore`도 import한다.

**다운그레이드 대화가 부르는 키는 `detect_downgrade`를 실제로 돌려 뽑는다**(detect_output·
detect_keys 픽스처). 손으로 적은 목록이면 스키마가 바뀌어도 초록이다 — 출력이 relpath
맵으로 바뀌었을 때 세 SKILL.md가 사라진 키를 읽는 채로 스위트 전체가 통과한 것이 실측이다.

**SKILL.md를 실행해서 재는 쪽 — 0단계의 플러그인 루트 해석 — 은 `test_script_root.py`에
있다.** 한 파일에 있던 그 둘을 갈랐다. 두 파일이 함께 쓰는 `SKILLS_DIR`·`SKILLS`는
`skill_paths.py` 한 곳에 있다 — 목록이 두 벌이면 아래
test_every_skill_on_disk_is_covered_by_the_contract가 한쪽만 덮는다.

**사용자 문서(README·backup-readme)는 `test_user_docs.py`에 있다** — 스킬도 스크립트도
아니어서 어느 쪽에도 속하지 않는다.

**`sync-restore/SKILL.md` 5-1~5-4의 「버킷 → CLI 명령」 결속은 `test_restore_prose.py`에
있다.** 그것은 배선(무엇을 어떤 순서로 부르는가)이 아니라 **실행 산문의 의미**(어떤
버킷이 어떤 명령에 실리는가)라 셋째 관심사다. 아래 `SCRIPT_CONTRACT_PHRASES`는 이 파일의
것이다 — 스킬이 스크립트의 어느 키를 읽는지를 거는 산문 앵커이고, 그쪽 파일은 그 키가
**어떤 행위에 묶이는지**를 건다.
"""
import json
import os
