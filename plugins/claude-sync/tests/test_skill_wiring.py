...
거는 것: 호환성 검사의 위치와 그것이 앞서야 할 실행줄, 다운그레이드 탐지 순서, 공유
스테이징을 비우는 횟수와 순서, base 게이트의 두 축, restore 명령의 스코프 정책, 스킬이
스크립트의 어느 키를 읽는지를 적은 산문 앵커, `## 동기화 대상` 절이 말하는 추출 필드,
선택 결과 JSON의 스키마, 그리고 그 표들이 스스로 줄어드는 것을 막는 완전성 메타가드.

그 계약을 재려면 프로덕션 어댑터의 상수가 필요해서 이 파일은 `plugin_config`를
import한다(선택 JSON의 섹션 이름, 보류 종류, 추출 필드 이름의 진실 원천). 4단계 bash와
같은 매칭 규칙을 대조하려고 `syncignore`도 import한다.
