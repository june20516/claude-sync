...
**그다음: 한 기기에서만 백업하고, 나머지 기기는 복원을 먼저 합니다.** 이 레포를 한 번도 reconcile하지 않은 기기에는 공유 기준선이 없어서, `/sync-restore`가 양쪽에 있는 항목을 모두 "양쪽이 모두 바뀌었습니다"로 보고하고 항목마다 묻습니다. 한 번 답하면 기준선이 기록됩니다. 그런 기기에서 `/sync-backup`을 먼저 하면 **묻지 않습니다** — 병합이 합집합으로 물러나면서 **양쪽에 있는 항목은 로컬 값이 레포 값을 조용히 대체하고, 충돌로 보고되지 않습니다.**
