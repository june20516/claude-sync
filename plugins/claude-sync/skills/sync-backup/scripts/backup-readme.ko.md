...
> - `plugins.json` — 그 기기의 `settings.json`에서 다시 만들어지는데, v2가 옮기는 것은 `enabledPlugins`와 `extraKnownMarketplaces` 둘뿐입니다. 플러그인·마켓플레이스 중 **다른 기기에만** 있는 것이 레포에서 사라지고, `pluginConfigs`와 `additionalMarketplaces`는 v2가 그 키를 아예 모르므로 **그 기기 것까지** 사라집니다.
>
> `sync-metadata.json`의 `min_reader_version` 표식은 이 백업을 이해하지 못하는 낮은 버전 기기를 막기 위한 것입니다. **정작 막아야 할 v2.x 기기에는 그 표식을 읽는 코드가 없습니다** — 그 가드는 v3.0.0에서 처음 들어왔기 때문입니다. 그래서 배포 순서가 유일한 방어입니다. 이미 벌어진 뒤에 되돌리려면 이 레포의 git 이력에서 그 두 파일을 복원해야 합니다.
...
이 파일은 **서버 이름 키 단위로 병합**되므로 한 기기에서 백업해도 다른 기기에만 있는 서버가 사라지지 않습니다 — **모든 기기가 v3.0.0 이상일 때에 한합니다.**

이 파일은 claude-sync v3.0.0 이상이 스키마 v2(`{"version": 2, "scope": "user", "servers": {...}}`)로 씁니다. **아직 v2.x인 기기는 자기 `claude mcp list` 출력으로 만든 배열로 이 파일을 덮어씁니다** — 위의 *백업하기 전에: 모든 기기가 v3.0.0이어야 합니다* 절을 보세요. 그 절은 `plugins.json`까지 함께 다룹니다.

### `plugins.json`에 대하여

`plugins.json`도 세 섹션 각각에 대해 같은 방식으로 키 단위 병합됩니다 — 플러그인 항목·마켓플레이스·설정 키가 각각 판정되므로 한 기기에서 백업해도 다른 기기에만 있는 항목이 사라지지 않습니다 — **모든 기기가 v3.0.0 이상일 때에 한합니다.**

**아직 claude-sync v2.x인 기기는 이 병합을 하지 않습니다** — 자기 `settings.json`에서 파일을 다시 만듭니다. 어느 기기에서든 백업하기 전에 위의 *백업하기 전에: 모든 기기가 v3.0.0이어야 합니다* 절을 보세요.
