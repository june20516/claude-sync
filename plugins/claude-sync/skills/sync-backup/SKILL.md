...
### 4.5 레포 문서 진단 — 다운그레이드·구문 손상

**수집 단계들보다 먼저 한다.** 5단계가 `plugins.json`을, 6단계가 `mcp-servers.json`을 v2로 덮어쓰면 "레포가 그 문서의 옛 형식이었다"는 증거가 사라져 탐지 자체가 불가능해진다. **두 문서 모두에 해당하므로 가장 앞선 수집 단계보다도 앞이다.** 구문이 깨진 문서도 여기서 잡는다 — 5·6단계가 그 문서를 건너뛰기 전에 되돌릴 기회를 주기 위해서다.
...
3. `newer_schema_seen`이 `true`면 히스토리에 **이 버전이 알아보지 못하는 백업**이 있다는 뜻이다. 그 사실을 알리고, 복구 후보로 제시된 커밋이 그보다 오래된 것임을 명시한다.
4. `downgrade_suspected`가 `false`면 그 문서는 조용히 넘어간다.
5. `downgrade_suspected`가 `true`면 레포의 `<relpath>`가 **그 문서의 옛 형식**인데 이 기기의 base는 v2였다는 뜻이다 — **옛 버전 기기가 덮어썼다.** 사용자에게 다음을 보여주고 고르게 한다. **문서마다 따로 묻는다** — 한 문서의 선택을 다른 문서에 적용하지 않는다.
...
6. `candidate`가 `null`이면 히스토리에 그 문서의 v2 커밋이 없다는 뜻이다. 사고는 알리되 복구는 제안하지 않는다.
...
- `"skipped"`: 셋 중 하나다 — `settings.json`을 읽지 못했거나, **레포 파일의 형식을 알아볼 수 없거나**(상위 버전이 쓴 백업일 수 있다), **레포 파일의 JSON 구문이 깨졌다.** 어느 갈래든 **레포의 `plugins.json`은 손대지 않았고 base도 전진시키지 않는다.** `reason`을 알리고 **그 문서만** 건너뛴다 — **6단계의 MCP 수집도 파일 동기화도 그대로 진행한다.** 이것은 섹션 단위 skip이 아니라 **문서 단위**이고(출력에 `sections` 자체가 없다), "항목 0개"가 아니다 — 백업된 것이 없다고 보고하지 않는다.
  ...
  `reason_kind`가 **`broken_syntax`**이면 레포 파일 자체가 손상된 것이다 — **플러그인 업데이트는 소용이 없다.** 4.5단계에서 복구하지 않아 이 문서를 건너뛴다 — "복구하려면 `/sync-backup`을 다시 실행해 4.5단계에서 「복구한다」를 고르세요"로 안내한다. 4.5단계에 후보가 없었던 경우에만 그 파일을 정상 JSON으로 되돌린 뒤 다시 실행하도록 안내한다. **그냥 지우라고 안내하지 않는다** — 그 문서에만 있던 다른 기기의 항목은 **이 기기의 로컬에 없어서 다음 백업이 되밀 수 없다** — 지우면 그 항목이 레포에서 사라진다.
...
- `"skipped"`: 셋 중 하나다 — `~/.claude.json`을 읽지 못했거나, **레포 파일의 형식을 알아볼 수 없거나**(상위 버전이 쓴 백업일 수 있다), **레포 파일의 JSON 구문이 깨졌다.** 어느 갈래든 **레포의 `mcp-servers.json`은 손대지 않았고 base도 전진시키지 않는다.** `reason`을 사용자에게 알리고 **그 문서만** 건너뛴다 — **5단계의 플러그인 수집도 파일 동기화도 그대로 진행한다.**
  ...
  `reason_kind`가 **`broken_syntax`**이면 레포 파일 자체가 손상된 것이다 — **플러그인 업데이트는 소용이 없다.** 4.5단계에서 복구하지 않아 이 문서를 건너뛴다 — "복구하려면 `/sync-backup`을 다시 실행해 4.5단계에서 「복구한다」를 고르세요"로 안내한다. 4.5단계에 후보가 없었던 경우에만 그 파일을 정상 JSON으로 되돌린 뒤 다시 실행하도록 안내한다. **그냥 지우라고 안내하지 않는다** — 그 문서에만 있던 다른 기기의 서버는 **이 기기의 로컬에 없어서 다음 백업이 되밀 수 없다** — 지우면 그 서버가 레포에서 사라진다.
