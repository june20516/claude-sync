...
최상위 `status`가 `"skipped"`면 **git 히스토리를 훑지 못했다**는 뜻이다. 최상위 `reason`을 알린다. **그것을 "다운그레이드를 확인하지 못했다"로 옮겨 적지 않는다** — 이때도 `files` 맵은 채워져 나오고 형태 비교는 git 없이 완결되므로 파일별 판정은 여전히 사실이다. 못 한 것은 후보 커밋 탐색이다. 아래 파일별 보고는 그대로 한다.
...
- `candidate`가 있으면 그 커밋의 날짜(`date`)와 `entries`의 **버킷별 항목 수**도 함께 보여준다. 버킷 이름은 문서마다 다르므로 **나온 키를 그대로 쓴다.** status는 복구하지 않는다.
