...
최상위 `status`가 `"skipped"`면 **git 히스토리를 훑지 못했다**는 뜻이다. 최상위 `reason`을 알리고 계속한다. **그것을 "다운그레이드를 확인하지 못했다"로 옮겨 적지 않는다** — 이때도 `files` 맵은 채워져 나오고 형태 비교는 git 없이 완결되므로 파일별 판정은 여전히 사실이다. 못 한 것은 후보 커밋 탐색이다.
...
| `plugins.json` | 이 기기에 없는 **타 기기의 플러그인·마켓플레이스·설정 키** (2.x는 이 파일을 통째로 다시 만든다) | **5-5** — "다른 기기가 지웠습니다" |
...
| `action_held` | 어떤 CLI 명령의 대상도 되지 않는다. 종류별로 한 번만 안내한다 (5-6) |
...
**`files["plugins.json"]`의 `downgrade_suspected`가 참일 때** — 케이스 4·5의 "다른 기기가 지웠습니다"는 **거짓이다.** 아무도 지우지 않았고, 낮은 버전 기기가 `plugins.json`을 통째로 다시 만들면서 **이 기기에 없는 타 기기의 항목을 흘린 것**이다. 그 문구는 사용자를 제거로 이끄는데, **이 항목의 마지막 사본이 지금 로컬에 있는 그것일 수 있다.** 대신 이렇게 쓴다.

> "이 항목은 다른 기기가 지운 것이 아니라, **낮은 버전 기기가 백업을 되돌리면서 레포에서 유실된 것으로 보입니다**(2.5단계 참조). 로컬에 남아 있는 이 값이 마지막 사본일 수 있습니다."
...
`sections[<섹션>]["action_held"]`에 있는 항목에는 **어떤 명령도 실행하지 않는다.** 종류별로 한 번만 안내한다.
