...
| `docs/superpowers/2026-08-20-plugins-sync-followup-BRIEF.md` | **착수됨.** 실측 완료(1-b·1-c), 결정 확정(5-a) |
| `docs/superpowers/2026-08-24-plugins-sync-STATUS.md` | **플러그인 작업의 현재 지점.** 이어받으려면 여기부터 |
...
**(6)·(7) 3.0.0에서 추가된 두 불변식**

버전 호환성 작업의 code review가 같은 결함을 반복해서 찾아낸 결과 승격됐다.
전문은 `specs/2026-08-21-version-compat-design.md` 10.0·10.0.1에 있다.

- **(6) 판정 불가를 통과로 접지 않는다.** "판정할 수 없다"는 표현 가능한 별도 상태여야 하고,
  모르는 입력의 기본 갈래는 통과가 아니라 차단이거나 예외다. 아홉 번 반복된 결함이며
  전부 "판정할 수 없는 상태를 문제 없음으로 접는" 한 형태였다 — 접두 정규식, 못 읽은 표식,
  없는 레포 경로, float 버전, 방어선 없는 main(), 호출자 오류를 삼킨 shape_of.
- **(7) 테스트는 의미 역전을 잡아야 한다.** 문서·설정 계약을 파일 전체에 대한 부분 문자열
  존재 검사로 확인하지 않는다. 절 단위로 자르고, 순서는 실행 줄을 앵커로 앞뒤를 둘 다 걸고,
  열거형은 원본에서 뽑아 대조한다. 세 번 반복됐고, 마지막에는 세 스킬의 차단·경고·질문을
  전부 뒤집어도 347개가 전부 통과했다.

둘의 근본 원인은 같다 — **손이 가장 덜 가는 방법이 작동하지 않는 방법이다.**
