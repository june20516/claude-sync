...
**Goal: 3.0.0을 배포 가능한 상태로 만든다** — 2.x가 섞인 함대에 breaking 릴리즈를 내보내도 **조용한 데이터 손실이 없는 상태.** 네 갈래다.

| 갈래 | 무엇을 없애는가 | task |
|---|---|---|
| **ⓐ 다운그레이드 사고** | 2.x 기기 한 번의 백업이 **타 기기 플러그인을 영구히 지우고**, restore가 *"다른 기기가 지웠습니다"* 라는 거짓 문구로 제거를 권하는 것. 예방(경고 산문) · 탐지(형태 판정) · 안내(대화) 셋이 다 필요하다 | 1·2·3·5·6 |
| **ⓑ 남은 로컬 손실 경로** | 비원자적 쓰기가 **잘린 파일을 레포의 온전한 사본 위에 push**하는 것, 요청하지 않은 로컬 상태 변경 | 8·9 |
| **ⓒ 검증되지 않은 자리** | 에뮬레이터가 못 돌려서 **한 번도 테스트되지 않은 복원 경로**(url·git 마켓플레이스), 미측정 | 7·13 |
| **ⓓ 보고·산문 층의 드리프트** | 목록이 조용히 줄어드는 것, 문자열 분기가 조용히 어긋나는 것, 사유를 말하지 못하는 보고 | 4·10·11·12 |

마지막에 **릴리즈 게이트(Task 14)** 가 릴리즈 계획 5장의 범위가 실제로 닫혔는지 검증한다. 푸시·배포는 외부 동작이므로 **사용자가 실행한다.**
...
> **사용자 결정(2026-08-31):** *"보이는 이슈는 다 수정하고 3.0.0 릴리즈한다. 이번엔 절대로 넘기지 않는다. 이번에 하기로 한 건 이번에 한다. 작업 중 발견되는 문제는 이번 목적에 방해가 된다고 판단되면 이번에 하고, 그게 아니라면 다음으로 넘기는 것 따위는 없다."*

그래서 이 plan은 **이월 표를 만들지 않는다.** 규칙은 둘이다.

1. **착수 시점에 이미 보이던 이슈는 전부 이 plan이 처리한다.** plan ②가 넘긴 14행을 한 줄씩 판정해 **10개는 task로 흡수**했고(10~13), **5개는 「닫은 것」 절에 이유와 함께 닫았다** — 넘기지 않았다.
2. **작업 중 새로 발견되는 것은 그 자리에서 판정한다.** 이 plan의 Goal(3.0.0을 안전하게 릴리즈)에 **방해가 되면 지금 한다.** 방해가 되지 않으면 **「닫은 것」에 이유를 적고 닫는다.** 새 이월 항목을 만들지 않는다.

**판단이 애매하면 미루지 말고 사용자에게 묻는다.** 내 기준으로 조용히 넘기는 것이 이 프로젝트가 반복해 온 실패다.
...
## 검증을 언제 멈추는가 — 명시적 기준 셋

**이 절이 없으면 검증이 끝나지 않는다.** 실측이 그것을 보였다 — Task 1의 리뷰 다섯 라운드에서
**프로덕션 로직 결함은 0건**이었고 발견은 전부 **검증 계층과 산문**이었다. 코드는 첫 시도에
수렴했는데 *"그 테스트를 무엇이 지키는가"* 가 수렴하지 않았다(spec r2가 잡은 것이 정확히
「테스트를 지키는 테스트가 비어 있었다」이다). 그 회귀에는 자연스러운 정지점이 없으므로
**기준을 명시한다.** 아래 셋은 전부 검사 가능하다 — "판단으로 끊는다"는 기준이 아니다.

### ⑴ 커버리지 기준 — 무엇을 다 하면 끝인가

> **각 task의 Step 4b는 위 표의 다섯 축 각각에 최소 하나의 변조를 세운다. 다섯이 전부
> CAUGHT면 그 task의 변조 검증은 끝이다.**
>
> 리뷰어가 새 변조를 찾아내면 그것은 **축이 하나 빠졌다는 신호**이지 무한 탐색의 근거가
> 아니다. **빠진 축을 채우고 멈춘다.**

**실측 근거.** Task 1의 quality 리뷰가 잡은 실제 결함(호출부의 relpath 오배선 — 틀린 값을 꽂아도
1045개가 초록이었고 실제 다운그레이드를 조용히 삼켰다)은 **1번 축「훅 호출 계약」**이다. Task 1의
Step 4b 목록이 그 축을 세우지 않아 리뷰 두 라운드가 더 돌았다. 다섯 축을 채웠으면 1라운드에
나왔다.

### ⑵ 깊이 기준 — 증명의 증명을 어디서 끊는가

> **가드는 자기 입력이 비어 있으면 스스로 실패한다**(선택자 위를 도는 단정은 그 선택자가
> 비었음을 자기 몸통에서 실패로 만든다). **가드를 지키는 별도의 가드를 만들지 않는다.**

이 규칙이 회귀를 **구조적으로** 닫는다. 새 층을 쌓으면 *"그 새 층이 비면?"* 이 다시 열리지만,
가드가 스스로를 검사하면 다음 층이 필요 없다.

### ⑶ 라운드 기준 — 언제 리뷰를 그만 도는가

> **한 라운드가 ⑴·⑵를 충족하고 프로덕션 로직 결함 0건이면 종료한다.** 남은 것이 이름·가독성
> 등 미등급뿐이면 그것은 라운드를 더 도는 근거가 **아니다.**

---

## 검증 강도는 위험도로 나눈다

**축은 「코드냐 문서냐」가 아니다.** 이 저장소에서는 `SKILL.md` 산문이 **실행물**이다 — 스킬이
읽고 그대로 행동하므로 `sync-restore/SKILL.md`의 한 문단이 사용자에게 "이 플러그인을
제거하시겠습니까"를 띄운다. 문서로 분류하면 **가장 위험한 자리가 가장 헐거워진다.**

옳은 축은 **"여기서 조용히 틀리면 되돌릴 수 없는 행동이 나가는가"**다.

| 등급 | 해당 | 대접 |
|---|---|---|
| **중** | 판정 로직, 쓰기 경로, **사용자에게 행동을 권하는 산문** | 변조 전량(다섯 축) + 리뷰 2단계 |
| **경** | 보고 형식, 사실 진술 docstring, spec 서술 | grep 전수 + 리뷰 1단계 |
| **미** | 이름·가독성·주석 다듬기 | 리뷰 없이 커밋 |

Task 1로 보면 `compat.py`의 판정은 **중**, spec의 옛 arity 서술은 **경**, `_rule_for` 이름은
**미**다. 실제로는 셋 다 중 대접을 받아 80줄짜리 프로덕션 변경에 1시간 53분이 들었다.

---

## 변조는 실행을 공유하고 설계는 공유하지 않는다

리뷰어에게 *"보고서를 신뢰하지 마세요"* 를 지시하는 근거는 옳다. 다만 **변조 결과는 커밋에
묶인 결정론적 산출물**이므로 재실행에서 새 정보가 나오지 않는다.

**실측.** Task 1에서 리뷰어가 만든 값은 **전부 자기가 새로 고안한 변조**에서 나왔고(실제 버그를
잡은 것은 quality 리뷰어의 W3다), implementer가 이미 돌린 변조를 다시 돌린 데서는 **새 정보가
0건**이었다.

- **공유할 것(실행):** 하네스 결과를 **커밋 SHA에 묶어** 남긴다. 리뷰어는 그 SHA가 검토 대상과
  같은지 **대조**하고, 이미 CAUGHT로 기록된 변조는 재실행하지 않는다.
- **공유하지 않을 것(설계):** 리뷰어는 **자기 변조를 추가**하는 데 시간을 쓴다. 특히 ⑴의
  다섯 축 중 implementer가 빠뜨린 축을 찾는다.

**APPLY_FAIL을 SURVIVED로 읽지 말 것.** 이름을 바꾸는 커밋은 기존 변조의 앵커를 낡게 만든다.
치환이 안 먹은 것을 "통과했으니 SURVIVED"로 읽으면 그 자리가 조용히 뚫린다(Task 1에서 실제로
한 번 발생했고 하네스의 `count` 검사가 걸렀다). **이름을 바꿨으면 기존 spec의 앵커를 함께 갱신한다.**
...
### Task 10: 산문 층의 자기 축소를 닫는다

**근거:** spec 13장 · 유지보수 이월(plan ②) — **등급: 중**(사용자에게 행동을 권하는 산문이 포함된다)

손으로 고른 목록은 **자기가 줄어드는 것을 탐지하지 못한다**(공허해지는 형태 ③). 양 언어에서 같은 항목을 개수와 함께 지우면 조용히 통과한다.

- [ ] **Step 1.** `SYNCIGNORE_MEANING`·`CORRECTIONS`에 **완전성 단정을 짝짓는다.** 바늘을 손으로 적지 말고 원천에서 뽑을 것 — 원천이 없으면 **무엇이 원천인지부터 정한다**(spec의 해당 절이 후보다).
- [ ] **Step 2.** **한국어 두 문서의 동시 편집 결속.** 리뷰가 40줄 프로토타입으로 **실측한 방법**이 있다: spec 13장의 한국어 불릿에 **같은 순번으로 묶는다.** 그 방법을 그대로 구현한다.
- [ ] **Step 3.** 옛 앵커 둘을 갱신한다 — `plans/2026-08-21-version-compat.md:2310`, `specs/2026-08-24-plugins-sync-design.md:1641`. **행 번호는 지금 다시 확인할 것**(이 plan을 쓰면서 spec 13장의 행 번호가 낡은 것을 이미 한 번 겪었다).
- [ ] **Step 4b (변조).** 다섯 축 전부. 입력 축이 이 task의 주역이다 — 목록에서 항목 하나 빼기, 양 언어에서 **동시에** 빼기, 선택자가 빈 값을 내게 하기, 순번 묶음을 어긋내기.
- [ ] **Step 5b (docstring).** 목록 옆 주석이 *"원천이 없다"*고 적고 있으면 Step 1이 그것을 거짓으로 만든다.

**영어 절반은 이 task의 범위가 아니다** — 「닫은 것」 절에 이유를 적었다.

**완료:** 0 failed. 위 변조가 전부 CAUGHT.

---

### Task 11: 보고 층 — 사유를 문자열이 아니라 구조로 만든다

**근거:** spec 9.2(개정 필요) · Task 8·7·3 리뷰 관찰(plan ②) — **등급: 중**(SKILL.md의 분기가 사용자 행동을 가른다)

- [ ] **Step 1.** **`SKILL.md`의 `reason` 문자열 분기를 `reason_kind` 필드로 바꾼다.** 지금은 산문 문구가 바뀌면 분기가 **조용히 안 맞게** 되고 스킬이 다른 경로를 탄다. 문구는 표시용으로 남기고 판정은 `reason_kind`가 한다.
- [ ] **Step 2.** **`/sync-status`의 `unrestorable`에 사유를 싣는다.** 지금은 키 목록뿐이라 *"의사 출처라 원래 불가능"*과 *"레포에 소스가 없으니 백업한 기기에서 올려라"*를 가르지 못한다 — **사용자가 할 일이 서로 다르다.** 어댑터가 훅 다섯째 키로 `reason`을 이미 준비해 뒀고 `plan_plugins.py`에 `unrestorable_reasons`가 이미 있다. **`spec 9.2를 먼저 고친다`** — 지금 spec은 status에 구별만 요구하고 사유는 요구하지 않으므로, 코드가 spec에 없는 동작을 하면 다음 감사가 결함으로 읽는다.
- [ ] **Step 3.** `conflicts`·`repo_ahead`의 보고 분할이 `collect_mcp.py`·`collect_plugins.py`에 **축자 중복**이다. 같은 소비자(SKILL.md)가 읽는 같은 모양인데 정의가 두 곳이다. **코어에 넣지 말 것**(코어는 값 무관 판정이지 보고 층이 아니다) — 두 스크립트가 공유하는 **보고 층 자리**를 만든다.
- [ ] **Step 4b (변조).** 다섯 축 전부. 특히 ① `reason_kind`와 문구를 맞바꾸기 ② 사유 없는 항목을 조용히 빠뜨리기 ③ 보고 분할을 한쪽만 고치기(중복이 실제로 없어졌는가) ④ 입력 축 — 갈래 하나를 픽스처에서 빼기.
- [ ] **Step 5b (docstring).** `unrestorable_reason`의 docstring이 *"갈래의 순서와 조건은 `_plugin_restorable`·`marketplace_arg`와 같아야 한다"*고 적는다 — Step 2가 그 계약을 소비자까지 넓힌다.

**완료:** 0 failed. `grep -rn '"reason"' plugins/claude-sync/skills/*/SKILL.md`가 **판정에 쓰이는 자리를 하나도 내지 않는다**(표시용만 남는다).

---

### Task 12: `excluded_in_repo`의 3-way 분류

**근거:** 유지보수 이월(plan ②) — **등급: 중**(제외된 파일에 대한 처방이 사용자 행동을 가른다)

`/sync-status`의 `excluded_in_repo`가 3-way 분류를 하지 않아 **항목별 처방을 적을 수 없다.**

- [ ] **Step 1.** `reconcile_restore.py`의 매핑을 `lib/`로 옮긴다. **순수 이동으로 시작할 것** — 옮기면서 규칙을 바꾸면 무엇이 깨졌는지 가릴 수 없다.
- [ ] **Step 2.** `excluded_in_repo`가 3-way 판정을 타게 하고, 버킷별 처방을 보고한다. **`2c4d4ad`가 restore 5절에 둔 열한 버킷의 처방 표가 문구의 원천이다** — 새로 지어내지 말 것.
- [ ] **Step 3.** 이동한 매핑의 소비자가 둘이 되므로 **한쪽만 고쳐지는 자리**가 생긴다. 두 소비자가 같은 매핑을 쓰는지 거는 테스트를 짝짓는다.
- [ ] **Step 4b (변조).** 다섯 축 전부. 특히 순수 이동이 정말 순수했는지(규칙 하나 바꾸기), 버킷 하나를 처방에서 빼기(입력 축).
- [ ] **Step 5b (docstring).** 옮긴 매핑의 docstring이 *"호출자는 하나다"* 류를 말하면 낡는다.

**완료:** 0 failed. `git diff`에서 Step 1이 **순수 이동**임을 보일 수 있다.

---

### Task 13: 남은 측정과 corpus 위생

**근거:** spec 14.5 #4 · 유지보수 이월(plan ②) — **등급: 경**(측정은 중, 개명·커밋은 미)

- [ ] **Step 1.** **`defaultEnabled`를 되읽는 파일을 실측한다.** 설치된 플러그인의 그 필드를 CLI가 어느 파일에서 읽는지가 미측정이라 `recheck_values`가 `assumed` 갈래를 남긴다. **방법은 3차 스모크와 같다** — 임시 HOME + 로컬 디렉토리 마켓플레이스. **`/sync-backup`이나 세 스킬은 절대 실행하지 말 것**(이 기기 캐시가 `2.0.0`이라 레포가 파괴된다). `claude plugin`을 임시 HOME에서 재는 것은 안전하다.
- [ ] **Step 2.** 측정 결과를 `docs/superpowers/2026-08-29-plugin-cli-smoke.md`에 **표와 함께** 기록한다(요약만 싣지 말 것 — 이 문서에서 요약이 자기 표보다 넓어 틀린 사고가 두 번 났다). 닫히면 `assumed` 갈래를 제거하고, 닫히지 않으면 **왜 못 쟀는지**를 적는다.
- [ ] **Step 3.** **변조 명세를 레포로 옮긴다.** 지금 `*.json`이 세션 스크래치패드에만 있어 corpus가 휘발성이고, 그래서 **이 plan의 완료 정의("Step 4b가 실제로 돌았다")를 검증할 수 없다.** task별 spec을 저장소에 커밋한다.
- [ ] **Step 4.** `test_mcp_state_machine.py`를 내용에 맞는 이름으로 개명한다(어댑터 셋을 도는 보류 상태 기계 시나리오다). **`git mv` + 참조 갱신**, 내용 변경 없음.
- [ ] **Step 4b (변조).** Step 1이 무언가를 닫았다면 그 자리에 다섯 축. 닫지 못했으면 이 Step은 해당 없음이고 그 사실을 적는다.
- [ ] **Step 5b (docstring).** `recheck_values`의 docstring이 *"매니페스트의 `defaultEnabled`를 되읽으면 없앨 수 있으나 **미측정**"*이라고 적는다 — Step 1의 결과가 이 문장을 반드시 바꾼다(닫히든 못 닫든).

**완료:** 0 failed. 변조 명세가 `git ls-files`에 나온다.

---

### Task 14: 릴리즈 게이트 — 3.0.0이 나갈 수 있는지 검증한다

**근거:** `docs/superpowers/2026-08-21-release-3.0.0-PLAN.md` 3·4·5장 — **등급: 중**

**이 task는 배포하지 않는다.** 푸시·배포는 외부 동작이므로 **사용자가 실행한다.** 이 task가 하는 것은 *"나가도 되는가"*에 증거로 답하는 것이다.

- [ ] **Step 1.** **릴리즈 계획 5장의 범위가 코드에서 실제로 닫혔는지 검증한다** — 결함 A(통째 덮어쓰기)·B(켬/끔 미감지)·C(예외 처리 부재)와 공용 키 단위 3-way 코어 추출. **plan ②의 선언을 근거로 삼지 말고 코드와 테스트로 짚을 것.** 셋 각각에 대응하는 테스트를 지목한다.
- [ ] **Step 2.** 4장 배포 절차의 사전 조건을 확인한다 — `plugin.json`·`.claude-plugin/marketplace.json`이 둘 다 `3.0.0`인가, `release/3.0.0`과의 관계, 태그 규칙.
- [ ] **Step 3.** **배포 순서 규칙이 사용자 문서에 도달했는지 확인한다** — Task 6이 고친 네 곳이 `plugins.json`도 말하는가. 이것이 11.2가 말하는 **유일한 예방**이다.
- [ ] **Step 4.** 사용자가 실행할 **체크리스트**를 문서로 남긴다. 명령을 그대로 붙여넣을 수 있는 형태로. **이 개발 기기의 캐시가 `2.0.0`이라 `/sync-backup`을 실행하면 안 된다**는 경고를 체크리스트 맨 앞에 둔다.
- [ ] **Step 5b (docstring).** 릴리즈 계획 3장이 *"작업 3이 남았다"*고 적는다 — 끝났으면 그 문장을 갱신한다.

**완료:** Step 1의 셋에 각각 테스트를 짚을 수 있다. 짚지 못하는 것이 있으면 **그것이 릴리즈 차단 사유**이고, 이 plan이 그 자리를 먼저 닫는다.
...
- [ ] **이월 표가 존재하지 않는다.** `grep -n "다음 plan으로 넘길\|다음으로 넘길\|이월" docs/superpowers/plans/2026-08-31-plugins-downgrade-compat.md` → **「닫은 것」 절과 이 규칙을 설명하는 자리 외에는 출력이 없다**
- [ ] **Task 14 Step 1의 셋(결함 A·B·C)에 각각 테스트를 짚을 수 있다.** 짚지 못하는 것이 릴리즈 차단 사유다
- [ ] 각 task의 Step 4b가 **다섯 축을 전부** 인스턴스화했고 SURVIVE가 남아 있지 않다. 변조 명세가 저장소에 커밋되어 있다(Task 13 Step 3)
- [ ] **배포·푸시는 하지 않았다** — 외부 동작이므로 사용자가 실행한다. 이 plan은 체크리스트까지만 만든다

## 닫은 것 — 하지 않기로 하고 이유를 적는다

**이월이 아니다.** plan ②가 넘긴 14행을 한 줄씩 판정해 10개는 Task 10~13이 흡수했고, 아래 다섯은 **닫았다.** 다시 목록에 올리려면 아래 이유가 거짓이 되었음을 먼저 보여야 한다.

| 닫은 항목 | 왜 닫는가 |
|---|---|
| `https://github.com/o/r.git`가 github인지 git인지 | **잴 수 없고 잴 필요도 없다.** 네트워크가 필요하고, **프로덕션에 도달 경로가 없다** — `marketplace_arg`가 github 출처에 내는 것은 `repo` 필드(`"o/r"`)이지 URL이 아니다(3차 스모크 14장) |
| **영어** 산문의 두 문서 동시 편집 탐지 | **원천이 없다.** 한국어 절반은 spec 13장의 한국어 불릿이 원천이 되어 Task 10 Step 2가 닫지만, 영어 문서에는 대응하는 원천 문서가 존재하지 않는다. 원천 없이 목록을 손으로 적으면 **그 목록이 다시 같은 결함**이 된다 |
| 고정 `.tmp` 이름의 동시 실행 안전성 | **고치면 더 나빠진다.** 코드베이스에 락이 하나도 없어(전수 grep 0건) 동시 실행을 전제하지 않는 설계와 일관되고, `mkstemp`로 바꾸면 잔존 파일 이름이 무작위가 되어 `.gitignore` 대응이 어려워지는 **역효과**가 있다(Task 1 리뷰 M3) |
| 보류 종류를 하나 더할 때 네 곳을 동시에 고쳐야 하는 것 | **다섯째 종류가 계획에 없다.** 어긋났을 때 나는 `ValueError`는 사고가 아니라 **설계된 fail-closed 가드**다(Task 5 리뷰 I-2) |
| `test_keyed_sync.py`의 `RECOGNIZE_HOOK_CALL`이 별칭 `ks.`를 하드코딩 | **발동 조건이 성립하지 않았다.** 리뷰의 처방 자체가 조건부였다 — *"면제 목록이 커지면 AST로 올릴 것"*. 지금 면제 목록은 커지지 않았고, `test_sync_state.py`의 `ss.ks` 몽키패치가 그 별칭을 못박고 있다(Task 3 리뷰 M-1) |

**이 절에 새 행을 더하려면** 이 plan의 Goal(3.0.0을 안전하게 릴리즈)에 방해가 되지 않음을 보이고 이유를 적는다. 방해가 되면 **task로 만든다.** 애매하면 사용자에게 묻는다.
