...
**이 본문에 남은 사후 정정은 「근거」이지 「이력」이 아니다.** task 절로 되돌아가 덧붙인 것이
많은데(`> [날짜 갱신]` 블록, `규정 정정 N째(PN)` 문단, Step 4b의 변조 목록), 그것들은 전부
*"이 단정이 왜 지금 이 형태인가"* 를 적는다 — 이 plan을 다시 돌리는 사람이 같은 결함을
되풀이하지 않으려면 필요하다. 반면 *"그때 이런 리뷰 라운드가 있었고 점수가 이랬다"* 류는
**독자가 없어서**(task 실행자는 `plan2-tasks/`의 분할 파일만 읽는다)
`docs/superpowers/2026-08-26-plan2-progress-HANDOFF.md`로 옮겼다. 애매하면 남긴다.
...
**다섯째 축(테스트가 준 목록을 뺀다)은 다섯 자리에 돌린다** — `CORRECTIONS`의 쌍 하나,
`CORRECTIONS`의 문서 하나, `USER_DOCS`의 문서 하나, `LIMITS_ANCHOR`의 문서 하나,
spec 불릿 추출기가 빈 목록을 내는 경우.

**아래 둘은 첫 라운드가 놓친 SURVIVE다.** 둘 다 닫았고, 지금 그 가드가 이 모양인 근거다.
...
**I1·I2를 닫은 뒤 다섯째 축은 여섯 자리에 돌린다** — `NEEDLE_SOURCES` 셋 각각,
`USER_DOCS`, 디스크 선택자, `limit_tokens`의 정규식.

**이 절에 남아 있던 구멍 하나와 변조 라운드 셋의 점수 기록은 인계 문서로 옮겼다** —
`docs/superpowers/2026-08-26-plan2-progress-HANDOFF.md`. 여기 두면 **독자가 없다**:
task 실행자는 `plan2-tasks/`의 분할 파일만 읽고, 아직 닫히지 않은 그 구멍의 독자는
task 실행자가 아니라 **다음 라운드**다.
