...
def collect(repo_path, staging_dir, settings_path=None, installed_path=None,
    ...
    "지문이 현재 레포 값과 일치"이므로 레포를 읽기 전에는 둘 다 항상 빈 집합이 되고,
    그러면 레포의 버전 제약이 true로 덮이고 6.4의 탈출구가 무증상으로 죽는다.
...
- `changed` — 양쪽에 있으나 값이 다름. **켬/끔 변경이 여기 포함된다.** 값이 확장 포맷이면 "버전 제약"으로 말한다
...
- `not_installed` — 확장 포맷 값 중 **아직 설치되지 않은** 것. "설치됩니다"와 "레포 값을 보존합니다"를 갈라서 말한다
...
`not_installed`인 항목에는 "보존합니다"만 말하지 않는다 — 아직 설치되지 않았으면 거짓이 된다.
