...
| | 무엇이 틀렸나 | 근거 | 이 개정 |
|---|---|---|---|
| ① | 9.3.6이 **레포 문서 「구문」 깨짐**을 규정하지 않았다. `ks.load_backup`이 그것을 `{}`로 degrade하는데 그 근거는 **backup 방향으로만** 쓰여 있다. restore에서는 레포가 통째로 빈 것처럼 보여, 파일 하나가 깨졌을 뿐인데 **"이 기기의 플러그인을 전부 지웁시다"** 가 최상위 `ok`와 함께 나간다 | 실측(9.3.6) | 9.3.6에 행을 넣고 **restore는 전체 skip**. backup 방향의 degrade는 유지(사용자 결정) |
...
| **레포 문서 「구문」 깨짐** | **전체 skip이 아니다** — `{}`로 degrade하고 병합을 진행한다. **restore와 반대다**(9.3.6이 그 비대칭을 규정한다) |
...
| **레포 문서 「구문」 깨짐 (JSON 파싱 실패)** | **같음 — 전체 skip**(4차 개정 ①). `status: "skipped"`, `reason`은 *"레포 문서의 구문이 깨졌다"*. **`local_stale` 제안을 내지 않는다** |
...
##### 구문 깨짐에서 backup과 restore가 갈리는 이유 (4차 개정 ①)

`ks.load_backup`은 **구문이 깨진 레포 파일을 예외가 아니라 빈 문서(`{}`)로 degrade한다.**
그 degrade의 근거는 **backup 방향으로만** 쓰여 있었다 — *"레포 파일 하나가 깨졌다고 백업
전체를 막지 않는다."* **restore에는 그 근거가 없다.**
...
| 방향 | 깨진 레포 문서 | 왜 |
|---|---|---|
| **backup** | `{}`로 degrade **(유지)** | 다음 백업이 그 자리에 **정상 문서를 쓴다.** 잘못 판정해도 사용자에게 파괴적 **제안**을 내지는 않는다 |
| **restore** | **전체 skip** | restore가 내는 것은 **사용자에게 실행을 권하는 제안**이다. 거짓 근거로 만든 파괴적 제안은 사용자가 승인하는 순간 되돌릴 수 없다 |

**두 방향의 비대칭이 규정의 핵심이다.** 같은 degrade가 한쪽에서는 자기 회복 경로 위에 있고
다른 쪽에서는 파괴적 제안의 입력이 된다. **읽지 못한 것을 "비어 있다"로 접으면 안 되는 자리**가
restore다 — 이 문서가 3.2·4.4·6.4에서 세 번 같은 규율을 적었고(*"읽기 실패와 0개를 구별한다"*),
이 갈래만 빠져 있었다.
...
> **backup 쪽 degrade의 한계는 측정해 두었다**(15장 오픈이슈 6). *"다음 백업이 그 파일을
> 정상 내용으로 되돌린다"* 는 **base가 없을 때만 참이다.** base가 있으면 레포의 모든 키가
> 케이스 4로 떨어져 **레포 문서가 `{}`가 된다**(실측). 이 개정은 사용자 결정에 따라 backup
> 동작을 **바꾸지 않고**, 근거 문장만 실측에 맞춰 좁힌다.
...
| 파일 | 무엇을 | 근거 |
|---|---|---|
| `lib/keyed_sync.py` | 구문 깨짐을 **호출부가 구별할 수 있게** 한다. `load_backup`의 `{}` degrade는 backup 계약이므로 그대로 두고, restore 쪽이 쓸 갈래를 따로 준다 | 9.3.6 |
| `skills/sync-restore/scripts/plan_plugins.py` | 구문 깨짐 → **전체 skip**(`status: "skipped"`). 3단계 판정을 **실행 시점 로컬 값**으로 | 9.3.6 · 9.3.1 |
| `skills/sync-restore/scripts/plan_mcp.py` | 같은 구문 깨짐 갈래. **코어를 공유하므로 같은 결함이 있다** | 9.3.6 |
...
6. **backup 방향의 `{}` degrade를 그대로 둘 것인가** (4차 개정에서 새로 열림).
   사용자 결정은 **유지**였고 이 개정은 그것을 따랐다. 다만 그 결정의 근거로 쓰여 있던
   *"다음 백업이 그 파일을 정상 내용으로 되돌린다"* 는 **base가 없을 때만 참이다**(실측).
   base가 있으면 레포의 모든 키가 **케이스 4**로 떨어져 병합 결과가 `{}`가 되고,
   그 문서가 레포에 쓰인다 — `status: "ok"`인 채로. 다음 백업도 같은 판정을 반복하므로
   **자기 회복이 아니라 안정된 소실**이다.
   재현: 로컬 `{"a@m": true}` + base `{"a@m": true, "other@m": true}` + 레포 파일
   `{ this is not json` → 레포에 `{"enabledPlugins": {}, …}`가 쓰이고 보고는
   `local_stale: ["a@m"]`. base가 비어 있으면 같은 입력이 `a@m`을 **push한다**(케이스 1).
   → **다음 라운드가 판단할 것.** 이 개정은 규정을 실측에 맞춰 좁히기만 했다.
