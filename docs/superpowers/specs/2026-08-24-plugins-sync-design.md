...
3. **1·2가 모두 불가능할 때만 지운다.** 임시 클론(`$TMPDIR/claude-sync-repo`)에서 지우면 다음
   ...
   - **사본을 언제 지우는가**(사용자 결정 2026-08-31). 그 사본은 **레포 파일과 같은
     내용**이므로 **레포에 없는 비밀이 새로 담기지 않는다** — `pluginConfigs`의 값은 이미
     `<REDACTED>`로 마스킹돼 있다. 급히 지울 이유가 없다는 뜻이고, 그래서 순서를 지킬 수
     있다. (「평문 비밀이 하나도 없다」로 넓히지 말 것 — `extraKnownMarketplaces`의
     `source`에는 url·git 출처가 그대로 들어간다. 레포를 private으로 두는 요구는 사본에도
     그대로 적용된다.)
...
- **`unrestorable`에는 사유를 함께 싣는다**(`unrestorable_reasons`, plan ③ Task 11).
  키 목록만 주면 *"의사 출처라 원래 불가능하다"* 와 *"레포에 소스가 없으니 백업한 기기에서
  올려라"* 를 가르지 못하는데 **사용자가 할 일이 서로 다르다** — 전자는 할 일이 없고
  후자는 다른 기기에서 `/sync-backup`을 돌리는 것이다. 사유의 정본은
  `unrestorable_reason` 하나이고 restore의 같은 이름 필드와 **같은 훅 묶음**에서 나온다.
  자유 함수에 `repo`를 따로 넘기면 판정(`restorable`)과 사유가 **다른 repo**를 볼 수 있고
  양쪽 다 무증상이다 — 같은 기기가 같은 키에 대해 두 스킬에서 다른 사유를 말한다
...
4. 설정 채우기          claude plugin install <id> --config k=v --scope user
                        ...
                        - **이 명령도 enabledPlugins 값을 쓴다**(2단계와 같은 규칙).
                          그래서 3단계보다 **먼저** 돈다
                        - **로컬 확장 값을 평탄화할 id는 뺀다** (아래 "4단계는 로컬
                          확장 값을 평탄화하지 않는다"). 뺀 것은 계획의
                          config_skipped_local_extended로 **로컬 값과 함께** 보고한다
3. 값 맞추기            claude plugin enable|disable <id> --scope user   ← 마지막
                        - 현재 상태와 다를 때만 호출한다 (멱등이 아니다).
                          **그 "현재 상태"는 실행 시점에 다시 읽은 로컬 값이다** (아래)
                        - 대상은 **재읽기(recheck)의 출력**이고 **설치 여부로 좁히지
                          않는다.** 이미 설치된 id도 로컬 `true`인데 레포가 false면
                          켜진 채 남는다 — 좁히면 그 항목이 영영 안 꺼진다.
                          계획의 disable_after_install은 **disable 방향의 예상**일 뿐이다
                          (enable 방향은 재읽기만 낸다)
```
...
**4단계는 로컬 확장 값을 평탄화하지 않는다**(사용자 결정 2026-08-31). 위 실측이 보인 것은
레포 값이 복원되지 않는다는 것만이 아니다 — 그 명령은 **로컬** `enabledPlugins` 값이 객체면
그것을 `true`로 평탄화한다(1차 스모크 3장. 배열은 보존된다). 레포 오염은 아니지만(H3가 값
축을 잡는다) **사용자가 요청하지 않은 로컬 상태 변경**이다. 그래서 계획은 그런 id를
`config_keys`에서 빼고 `config_skipped_local_extended`에 **로컬 값과 함께** 싣는다. 값을 함께
싣는 것은 사용자가 무엇을 지키려고 건너뛰었는지 봐야 *"이 기기 값을 포기하겠다"* 를 고를 수
있어서다. 그 값은 정규화를 거친 `enabledPlugins` 값이라 비밀이 아니다(`pluginConfigs`가 아니다).
**이것은 실패가 아니다** — 건너뛴 것이므로 10.2의 갈래 구분과 같은 종류로 렌더링한다.

**배열도 함께 뺀다.** `install`이 배열을 보존하는 것은 실측이지만(2차 스모크 7장 6행) 값
종류로 가르면 표가 하나 더 생기고, H3가 배열·객체를 구분하지 않는다(`value_command`가
비불리언 전부에 `None`이다). 한 규칙이 좁고 안전하다.

**지킬 값이 있을 때만 뺀다.** 3단계가 그 키에 `enable`/`disable`을 낼 예정이면 그 명령이
확장 값을 **어차피** 지우므로(위 "왜 3단계를 마지막으로 옮겼는가"의 표) 빼도 최종 상태가
같고 설정만 채워지지 않는다 — 순수한 손해다. 그래서 판정은 `value_command(로컬 값, 레포 값)`이
`None`인 키로 좁힌다: 레포 값이 비불리언(H3)이거나 레포 `enabledPlugins`에 키가 없을 때다.
그 갈래의 3단계는 부수 효과가 아니라 레포와 갈린 값을 맞추는 **요청된 수렴**이다.

**판정은 로컬 값이고 레포 값이 아니다.** 레포 쪽 비불리언은 H3(값 보류)이고 *"밀 CLI가 없다"*
는 다른 사실이다. 좌우를 바꾸면 지켜야 할 로컬 값이 평탄화되고 지킬 것이 없는 id의 설정은
채워지지 않는다 — 두 방향 다 조용하다. **키 부재는 확장 값이 아니다**(9.3.3의 위임 상태다).
**설치 여부로는 여전히 좁히지 않는다** — 좁히는 축은 값 하나다.
...
1. **거짓 실패가 사라진다.** 레포가 `false`이고 매니페스트도 `defaultEnabled: false`인
   **미설치** id는 2단계 `install`이 `false`를 써 두므로, 실행 시점에 다시 읽으면 값이
   같아 **명령이 아예 나가지 않는다**(스모크 7장 귀결 3). *(초판은 이 자리에 "계획이
   부재를 `true`로 추정해 `disable_after_install`에 싣는다"고 적었다. 스모크 4차 18장이
   부재를 꺼짐으로 쟀으므로 계획 시점 판정도 이제 같은 답을 낸다 — 두 층이 일치한다.)*
2. **`defaultEnabled: true`인 다수에서 수렴이 돌아온다.** 2·4단계가 `true`로 덮어도
   3단계가 마지막에 레포의 `false`를 적용한다.
3. **추정 갈래가 사라졌다**(스모크 4차, 2026-09-01 — 14.5 #4가 닫혔다). 초판은
   *"2·4단계 어느 명령의 대상도 아니면서 로컬 `enabledPlugins`에 키가 없는 id는 읽을 값이
   없어 추정(`true`)에 머문다"* 고 적고, 그 추정이 틀렸을 때의 exit 1을 10.2가
   *"이미 그 상태입니다"* 로 구별하게 했다. **부재의 뜻이 측정되면서 추정 자체가 없어졌다** —
   CLI는 키가 없는 id를 **꺼짐**으로 읽는다(`disable`은 exit 1, `enable`은 exit 0으로 키를
   만든다. 설치 여부와 무관하다). `defaultEnabled`는 **install 시점에 한 번** 쓰여
   `settings.json`으로 물질화될 뿐 어느 파일에서도 되읽히지 않는다(4차 16장).

   그래서 재읽기는 부재를 `false`로 **읽고**, 두 방향이 모두 옳아진다 — 레포가 `true`이면
   `enable`이 나가고(초판은 아무 명령도 내지 않아 **레포의 `true`가 조용히 복원되지
   않았다**), `false`이면 명령이 나가지 않는다(초판은 `disable`을 내 거짓 실패였다).
   계획 층의 `assumed` 표식과 10.2의 그 갈래는 함께 제거됐다.
...
미설치가 아니라 **꺼짐**이므로(실측 — 스모크 4차 18장), 그것으로 가르면 이미 설치된
플러그인이 2단계로 간다.
...
**레포의 부재는 `false`가 아니다.** 레포에 키가 아예 없는 항목을 `disable` 대상으로 삼지
않는다 — 레포가 그 값에 **의견을 갖지 않은** 상태여서 맞출 대상이 없기 때문이다.

**로컬의 부재는 다르다.** 이 절의 구별(부재 = 지웠다 / `false` = 껐다)은 **동기화 층**의
것이고, **CLI 명령의 층에서는 로컬의 부재가 꺼짐이다**(실측 — 스모크 4차 18장:
`disable`은 exit 1, `enable`은 exit 0으로 키를 만든다. 설치 여부와 무관하다). 두 층을
섞으면 3단계가 틀린다 — 그 자리의 규정은 9.3.1이다.
...
손실 범위는 실측이고 **두 문서가 같은 형태로 무너진다** — 2.x는 레포를 읽지 않고 두 문서를
**그 기기 것만으로 통째로** 다시 만든다.

- `plugins.json` — `extract_plugins.py`가 로컬 settings에서 `enabledPlugins`와
  `extraKnownMarketplaces` **둘만** 복사한다. 타 기기의 플러그인·마켓플레이스가 사라지고,
  2.x가 아예 모르는 `pluginConfigs`·`additionalMarketplaces`는 **그 기기 것까지** 사라진다.
- `mcp-servers.json` — `parse_mcp.py`가 그 기기의 `claude mcp list` 출력만으로 배열을
  만든다. 타 기기에만 등록된 서버가 사라지고, 명령에 공백이 든 서버는 정규식이 놓쳐
  **그 기기 것까지** 사라진다.

**표식은 이것을 막지 못한다.** `compat._block_reason`은 `mine < required`일 때만 막는데
표식 값이 `MIN_READER_VERSION`(3.0.0)이라 3.0.0 기기는 통과하고, 막아야 할 2.x에는 표식을
읽는 코드가 아예 없다 — 그 가드는 3.0.0에서 처음 들어왔다. 그래서 *"v3 기기가 표식을 읽고
멈춘다"* 는 서술은 거짓이며, 다섯 문서의 표식 문단은 그 사실대로 적는다.
...
| `sync-restore/SKILL.md` 2.5절 | 같음 | **닫혔다**(Task 5) — 다운그레이드 대화를 파일 맵 위에 다시 세우면서 손실 방식이 문서별 표가 됐다. **표의 mcp 행은 Task 6이 함께 넓혔다** — 2.x의 `parse_mcp.py`가 그 기기의 `claude mcp list` 출력만으로 배열을 통째로 만들므로 타 기기에만 등록된 서버도 사라진다(실측). 같은 사실을 말하는 여섯 자리 중 하나만 좁게 남기면 드리프트가 그대로 재현된다 |
...
| `enable` / `disable` | 현재 상태와 다를 때만 exit 0. **현재 상태는 `enabledPlugins`의 값 하나로 정해진다** — `true`면 켜짐이고 `false`·비불리언·**키 부재**는 전부 꺼짐이다(실측 — 스모크 4차 18장. 설치 여부와 무관하다). `enable`은 꺼짐인 id에 **키를 만든다**(미설치 id에도 — 유령 키). `disable`은 2.1.252에서 이미 꺼진 id에 exit 1이고 키를 만들지 않는데, **1차(2.1.250)는 exit 0으로 `{id: false}`를 만들었다** — 판본 차이이고 에뮬레이터는 더 위험한 1차 동작을 재현한다 |
...
**3차 스모크(2026-08-31, CLI 2.1.251)가 #1·#2·#3을, 4차(2026-09-01, CLI 2.1.252)가 #4를
닫았다.** 같은 문서 11~14장과 16~19장이다.
...
| ~~4~~ | **설치된 플러그인의 `defaultEnabled`를 어느 파일에서 되읽는가** | **닫힘 — 되읽지 않는다.** install 시점에 한 번 쓰여 `settings.json`으로 물질화된다. 그 뒤의 키 부재는 CLI에게 **꺼짐**이다 (4차 16·18장) |
