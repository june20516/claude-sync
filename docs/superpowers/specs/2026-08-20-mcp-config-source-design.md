...
def restore_plan(local: dict, backed: dict, base: dict | None) -> dict
    """{"add": [...], "needs_secret": [...], "in_sync": [...],
        "differs": [...], "local_stale": [...]}.
    local_stale은 로컬에 있고 레포에 없으며 base에 있는 서버 — 케이스 4와 5를 **모두** 담는다.
    merge.local_stale(케이스 4만) ⊆ restore_plan.local_stale(케이스 4+5)이다.
    케이스 5까지 담지 않으면 backup은 매번 "충돌 — /sync-restore 먼저"라고 안내하는데
    restore가 아무것도 보여주지 않는 탈출구 없는 상태가 된다.
...
- 갱신 시점:
  - backup — 커밋·푸시 **성공 이후에만** `next_base`를 기록한다. 푸시가 실패하면 레포가 실제로
    그 내용을 갖지 않으므로 base도 전진해서는 안 된다(`update_base.py`와 같은 계약).
  - restore — MCP 단계 이후 `_next_base(복원 후 로컬, 이전 base, 레포)`를 기록하고,
    "유지"를 선택한 이름은 base에서 제거한다(7.4).
...
- **타 기기 선행**(케이스 2·8): `repo_ahead`로 보고한다. 파일 단위 `reconcile_backup.py`는
  ...
  "`/sync-restore`를 실행하면 로컬에 반영됩니다"를 안내한다.
- restore: additive이므로 이미 등록된 서버는 **추가**하지 않지만, 케이스 8은 7.7의 선택지를 제시한다.

### 7.7 케이스 8의 해소 — `differs`에도 대칭 처방이 필요하다
...
| 선택 | 동작 | 다음 backup | 도달 상태 |
|---|---|---|---|
| **레포 값 채택** | `claude mcp add-json`으로 로컬 덮어쓰기, `base[x] ← 레포 값` | 케이스 6 | 양쪽 레포 값 |
...
merge(L, R, S) → servers를 dump_backup → 레포
푸시 성공 → next_base를 base 블롭으로 기록
```
...
- `conflicts` → "`/sync-restore` 먼저 실행하세요"
- `local_stale` → "`/sync-restore`에서 로컬을 정리하세요"
- `repo_ahead` → "다른 기기가 추가·변경했습니다. `/sync-restore`를 실행하면 로컬에 반영됩니다"
...
local_stale:  "다른 기기에서 삭제된 서버가 로컬에 남아 있습니다."
              ...
              유지   → 로컬 그대로 두고 base에서 그 이름을 제거 (다음 backup에서 레포로 push)
              나중에 → 아무것도 하지 않음
base ← _next_base(복원 후 로컬, 이전 base, 레포),  "유지" 선택 이름은 base에서 제거
```
...
- `base=None`(첫 백업) 분기가 `repo_ahead`를 채우지 않고, 같은 이름의 값이 다르면 보고 없이
  로컬 승으로 확정된다. 첫 백업 한정이라 Minor로 두되, Task 8 결과 보고에 "첫 백업이라
