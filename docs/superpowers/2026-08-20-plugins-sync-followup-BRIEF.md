...
실측으로 확인한 것 (**2026-08-24 2차 측정으로 보강** — 1차에서 "배열도 평탄화된다"로
잘못 일반화했고, 그 오류가 spec 초판에 그대로 실렸다):

| 값 형태 | 그 플러그인을 `install` | 결과 |
|---|---|---|
| 배열 `["1.0.0"]` | 미설치 → 설치 | **보존됨** |
...
**평탄화는 객체 형태 한 갈래뿐이다.** 배열은 CLI를 통과해도 살아남는다.

**아직 모르는 것:** CLI가 "확장 포맷"으로 의도한 형태가 배열인지 객체인지, 객체 평탄화가
정규화인지 손실인지. 스키마 설명문은 형태를 밝히지 않는다 — 실환경 스모크의 측정 항목으로 남긴다.
...
**D3. `auto: true` 항목은 백업 대상에서 뺀다. 로컬 상태는 두 파일에서 읽는다.**

근거는 N6이다 — 복원이 `auto` 항목을 명시적으로 설치하면 표식이 지워져
**의존성이 영구히 수동 설치로 승격**되고, 부모를 지워도 `prune`이 청소하지 못한다.
반대로 빼는 것은 **무손실**이다: 부모는 수동 설치라 백업에 실리고, 복원이 부모를 설치하면
CLI가 자식을 `auto: true`인 채로 다시 끌고 온다.

대가는 **로컬 상태의 정의가 `settings.json` 단독이 아니게 되는 것**이다(5장 항목 5의 답):

- `~/.claude/settings.json` — `enabledPlugins`(+`additionalMarketplaces` 별칭),
  `extraKnownMarketplaces`, `pluginConfigs`. **동기화 대상 값의 원천.**
- `~/.claude/plugins/installed_plugins.json` — **`auto` 플래그만** 읽는다. 그 외에는 쓰지 않는다.
  (`settings.json`에서 파생되는 파일이므로 값의 원천으로 삼지 않는다 — 1-c C5.)

`installed_plugins.json`을 읽지 못하면 **auto 판정이 불가능**하다. 불변식 2·6에 따라 통과로
접지 않고 **전량 포함 + 명시적 경고**로 처리한다 — 백업에서 빠뜨리는 것은 되돌릴 수 없는
손실이지만, `auto` 혼입은 복원 안내로 완화할 수 있기 때문이다. 조용히 넘어가지 않는다는 것이 요점이다.
